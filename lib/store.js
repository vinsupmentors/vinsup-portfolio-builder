...
export async function readStudent(slug) {
  ...
  let s = await readJson(DIR + slug + '.json', true);
  // if it looks unapproved (or missing), double-check with a fresh read so
  // approvals show up instantly instead of waiting for the cache to expire
  if (!s || s.status !== 'approved') {
    const fresh = await readJson(DIR + slug + '.json', false);
    if (fresh) s = fresh;
  }
  if (s) return s;
